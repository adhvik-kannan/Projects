...
public class Ship implements MouseMotionListener, KeyListener {
	...
	public Ship(int x, int y) {
		this.x = x;
		this.y = y;
		laser = new SoundPlayer("jrJava/alienInvader/imagesAndSounds/laser.wav");
	}
	...
	public void shootTorpedo() {
		...
		// TorpedoManager.add(new Torpedo(x+20, y-Torpedo.height + 40, -7));
		// TorpedoManager.add(new Torpedo(x-20, y-Torpedo.height + 40, -7));
	}
	...
	public boolean isHit(int xBC, int yBC, int w, int h) {
		return xBC >= x - topWidth / 2 - w / 2 && xBC <= x + topWidth / 2 + w / 2 && yBC >= y + 0 * eachHeight
				&& yBC <= y + 1 * eachHeight + h
				|| xBC >= x - middleWidth / 2 - w / 2 && xBC <= x + middleWidth / 2 + w / 2 && yBC >= y + 1 * eachHeight
						&& yBC <= y + 2 * eachHeight + h
				|| xBC >= x - bottomWidth / 2 - w / 2 && xBC <= x + bottomWidth / 2 + w / 2 && yBC >= y + 2 * eachHeight
						&& yBC <= y + 3 * eachHeight + h;
	}
