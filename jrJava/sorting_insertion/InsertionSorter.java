...
public class InsertionSorter {
	...
	// Performance:
	// 1. {17, 11, 8, 6, 5, 2}, how many comparisons? 1 + 2 + 3 + 4 + 5 --> 15
	// (worst-case)
	// 2. {2, 5, 6, 8, 11, 17}, how many comparisons? 1 + 1 + 1 + 1 + 1 --> 5
	// (best-case)
	...
	public static void sort(int[] data) {
		...
		for (i = 1; i < data.length - 1; i++) {
			...
			// The last element in the window is the new data and is probably in the wrong
			// spot.
			...
			// Being here means:
			// 1. It found the insertion point (found a data that is less than or equal to
			// the new data). In that case, the insertion point will be data[j+1].
			// 2. It could not find a data that is less than or equal to the new data and
			// used up all data and gets out of the for-loop. (it will come out with j=-1).
			// In that case, the new data should be inserted at data[0].
