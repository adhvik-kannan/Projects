...
public class RealNastyAlien extends UnfriendlyAlien {

	protected float transparency = 1.0f;
	protected float cloakRate = 0.015f;

	public RealNastyAlien(int x, int y, Image image, Image imageAlt, int vx, int vy) {
	...
	public void draw(Graphics g) {
		...
		drawCount++;
		if (drawCount % drawCycle < drawCycle / 2)
			g.drawImage(image, x - size / 2, y - size, size, size, null);
		else
			g.drawImage(imageAlt, x - size / 2, y - size, size, size, null);
