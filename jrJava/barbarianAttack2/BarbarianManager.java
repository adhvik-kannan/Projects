...
public class BarbarianManager {
	...
	private static void launchBarbarian() {
		...
		if(Math.random()>0.333) {
			barbarians.add(new RunningBarbarian(imagePairs[(int) (Math.random()*4)], -imagePairs[0][0].getWidth(null), 600, (int)(Math.random()*4+1)));
		}
		else {
			barbarians.add(new FlyingBarbarian(imagePairs[(int) (Math.random()*2)+4], -imagePairs[5][0].getWidth(null), (int)(Math.random()*400 + 100), (int)(Math.random()*5 +2), 50));
		}
	}
