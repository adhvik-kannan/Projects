...
public class Graveyard {
	...
	private static void setColorAt(int x, int y, Color color) {
		int i = (x - X_LEFT) / Block.SIZE + cols * (y - Y_TOP) / Block.SIZE;
		colors[i] = color;
	}

	public static void bury(Block[] blocks) {
		// It needs to sort the blocks in the increasing y-values.
		// The following is faked bubble sorting (close, not quite)
		Block temp;
	...
	public static void bury(Block block) {
		...
		// If so, we need to shave off the row (shift all the rows above down by one).
