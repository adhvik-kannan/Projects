...
public abstract class TetrisObject implements KeyListener {
	...
	// This method will check if any of the blocks in tetris object
	// happens to be outside the grave yard boundary.
	// It also checks if it overlap with already occupied blocks of
	// the grave yard.
	protected boolean isIllegalMove() {
		for (int i = 0; i < blocks.length; i++) {
			if (blocks[i].getX() < Graveyard.X_LEFT || blocks[i].getX() >= Graveyard.X_RIGHT
					|| blocks[i].getY() >= Graveyard.Y_BOTTOM
					|| Graveyard.isOccupied(blocks[i].getX(), blocks[i].getY()))
				return true;
		}
		return false;
	}
