package jrJava.maze_afterClass5;
...
public class Mouse implements Runnable, KeyListener {
	...
	private boolean isCollided() {
		...
		if (xSpeed != 0) { // horizontal move
			...
			for (int yProbe = y - size / 2 + Maze.MIN_THICKNESS; yProbe <= y + size / 2
					- Maze.MIN_THICKNESS; yProbe += Maze.MIN_THICKNESS) {
				if (maze.hitWall(xProbe, yProbe))
					return true;
			}
		} else { // vertical move
			int yProbe = y + ySpeed * size / 2;
			for (int xProbe = x - size / 2 + Maze.MIN_THICKNESS; xProbe <= x + size / 2
					- Maze.MIN_THICKNESS; xProbe += Maze.MIN_THICKNESS) {
				if (maze.hitWall(xProbe, yProbe))
					return true;
			}
		}
