	...
	public static void printMaxMin(Comparable[][] comps) {
		...
		for(i=0; i<comps.length; i++) {
			for(j=0; j<comps[i].length; j++) {
				...
				if(comps[i][j]==null) continue;
				if(comps[i][j].compareTo(max)>0) max = comps[i][j];
				if(comps[i][j].compareTo(min)<0) min = comps[i][j];
			}		
		}
	...
	public static void transferData(Object[] objects, Object[][] objs) {
		...
		for(i=0; i<objs.length; i++) {
			for(j=0; j<objs[i].length; j++) {
				if(count>=objects.length) objs[i][j] = null;
				else objs[i][j] = objects[count++];
			}
