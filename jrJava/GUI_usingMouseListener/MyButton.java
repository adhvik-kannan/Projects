...
public class MyButton extends JButton {
	...
	public MyButton() {
		...
		vx = new int[] { 3, 3, 4, 4, 5, 5, 6, 6, 7, 7 };
		vy = new int[] { 4, -4, 5, -5, 6, -6, 7, -7, 8, -8 };
		radius = new int[] { 3, 5, 7, 9, 10, 8, 6, 4, 7, 8 };
		colors = new Color[] { Color.RED, Color.GREEN, Color.BLUE, Color.MAGENTA, Color.ORANGE, Color.PINK,
	...
	public void moveCircle() {
		for (int i = 0; i < radius.length; i++) {
			x[i] += vx[i];
			y[i] += vy[i];
			if (x[i] - radius[i] < 10 || x[i] + radius[i] > getWidth() - 10)
				vx[i] = -vx[i];
			if (y[i] - radius[i] < 10 || y[i] + radius[i] > getHeight() - 10)
				vy[i] = -vy[i];
		}
	}
