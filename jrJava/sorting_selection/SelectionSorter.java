...
public class SelectionSorter {
	...
	// 4. If the size of the array is n, how many comparisons? (n-1) + (n-2) + ... + 1
	// 	  --> n(n-1)/2 --> ~n*n/2 --> O(n^2)
	...
	public static void sort(int[] values) {
		...
		// We shrink window size by one index at a time. Window range [0, len-1] --> [len-2, len-1].
		// In each window, we find (select) the minIndex (index at which the min is stored).
